...
function Chart({ data, clients, hoveredClient, showIndividual }) {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const ro = new ResizeObserver((entries) => { setDims({ w: Math.max(entries[0].contentRect.width, 300), h: 300 }); });
    ro.observe(el);
    return () => ro.disconnect();
  }, []);

  const padL = 70, padR = 20, padT = 20, padB = 50;
  const chartW = dims.w - padL - padR, chartH = dims.h - padT - padB;
  ...
  const handleMouseMove = (e) => {
    ...
    const idx = Math.round(((e.clientX - rect.left - padL) / chartW) * (data.length - 1));
    if (idx >= 0 && idx < data.length) setTooltip({ idx, x: xScale(idx) });
  };
  ...
  return (
    <div ref={containerRef} style={{ position: "relative", width: "100%" }}>
      <svg width={dims.w} height={dims.h} style={{ display: "block", cursor: "crosshair" }} onMouseMove={handleMouseMove} onMouseLeave={() => setTooltip(null)}>
        ...
        {data.map((d, i) => i % labelInterval === 0 ? <text key={i} x={xScale(i)} y={dims.h - 12} textAnchor="middle" fill="rgba(255,255,255,0.35)" fontSize="10" fontFamily="'DM Sans',sans-serif">{new Date(d.date).toLocaleDateString("fr-FR", { day: "2-digit", month: "short" })}</text> : null)}
        ...
        <div style={{ position: "absolute", left: Math.min(tooltip.x + 12, dims.w - 200), top: 30, background: "rgba(15,15,30,0.95)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 10, padding: "10px 14px", zIndex: 10, backdropFilter: "blur(12px)", minWidth: 170 }}>
          ...
          {clients.map((c, ci) => (
            <div key={c.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "rgba(255,255,255,0.55)", padding: "1px 0" }}>
              <span style={{ display: "flex", alignItems: "center", gap: 5 }}><span style={{ width: 7, height: 7, borderRadius: "50%", background: PALETTE[ci % PALETTE.length], display: "inline-block" }} />{c.name}</span>
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={x + barW / 2} y={y - 4} textAnchor="middle" fill="rgba(255,255,255,0.5)" fontSize="9" fontFamily="'JetBrains Mono',monospace">{formatNum(d.sessions)}</text>
              <text x={x + barW / 2} y={chartH + 24} textAnchor="middle" fill="rgba(255,255,255,0.3)" fontSize="9" fontFamily="'DM Sans',sans-serif">{label}</text>
            </g>
          );
        })}
      </svg>
...
function EvoBadge({ value, label }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
      <span style={{ fontSize: 14, fontWeight: 700, color: isPos ? "#6ee7b7" : "#fca5a5", fontFamily: "'JetBrains Mono',monospace" }}>
        {isPos ? "▲" : "▼"} {formatPct(value)}
      </span>
...
export default function GA4Dashboard() {
  ...
  const [summaryCustom, setSummaryCustom] = useState(null);
  ...
  // Fetch traffic
  const fetchTraffic = useCallback(async (customStart, customEnd) => {
    if (!auth.authenticated) return;
    setTrafficLoading(true); setTrafficError(null);
    try {
      let url;
      if (customStart && customEnd) {
        url = `${API_URL}/api/traffic?startDate=${customStart}&endDate=${customEnd}`;
      } else if (customRange) {
        url = `${API_URL}/api/traffic?startDate=${customRange.start}&endDate=${customRange.end}`;
      } else {
        url = `${API_URL}/api/traffic?days=${period}`;
      }
      const res = await fetch(url, { credentials: "include" });
      if (res.status === 401) { setAuth({ authenticated: false, email: null, checking: false }); return; }
      const json = await res.json();
      if (!json.success) throw new Error(json.error);
      setTrafficData(json);
    } catch (err) { setTrafficError(err.message); }
  ...
  const handleLogout = async () => {
    ...
    setAuth({ authenticated: false, email: null, checking: false }); setData([]); setClients([]); setTrafficData(null);
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d1a", color: "#e2e2f0", fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
      <style>{`* { margin: 0; padding: 0; box-sizing: border-box; } html, body, #root { width: 100%; min-height: 100vh; overflow-x: hidden; }`}</style>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12, marginBottom: 12 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
              ...
              <h1 style={{ fontSize: 20, fontWeight: 700, margin: 0, letterSpacing: "-0.02em" }}>Dashboard GA4</h1>
            </div>
      ...
      <div style={{ padding: "24px 28px", maxWidth: 1200, margin: "0 auto" }}>
          ...
          <>
            ...
            {loading ? (
              <div style={{ textAlign: "center", padding: "60px 0", color: "rgba(255,255,255,0.3)" }}>
                <div style={{ fontSize: 24, marginBottom: 12, animation: "spin 1s linear infinite" }}>⟳</div>Chargement...
                  ...
                  <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, padding: "20px 20px 12px", marginBottom: 24 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                      ...
                      <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "rgba(255,255,255,0.4)", cursor: "pointer" }}>
                        <input type="checkbox" checked={showIndividual} onChange={(e) => setShowIndividual(e.target.checked)} style={{ accentColor: "#6366f1" }} />Détail
                      </label>
                    </div>
                  ...
                  <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, overflow: "hidden" }}>
                    ...
                    <div style={{ overflowX: "auto" }}>
                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                        <thead><tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>{["Client", "Propriété", "Revenu", "Part", "Tendance"].map((h) => <th key={h} style={{ padding: "10px 16px", textAlign: h === "Revenu" || h === "Part" ? "right" : "left", fontSize: 11, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "0.05em", fontWeight: 600 }}>{h}</th>)}</tr></thead>
                        <tbody>
                          {clientStats.map((c) => (
                            <tr key={c.id} onMouseEnter={() => setHoveredClient(c.id)} onMouseLeave={() => setHoveredClient(null)} style={{ borderBottom: "1px solid rgba(255,255,255,0.03)", background: hoveredClient === c.id ? "rgba(99,102,241,0.05)" : "transparent", cursor: "pointer" }}>
                              ...
                              <td style={{ padding: "12px 16px", textAlign: "right" }}>
                                <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 6 }}>
                                  <div style={{ width: 60, height: 6, borderRadius: 3, background: "rgba(255,255,255,0.06)", overflow: "hidden" }}><div style={{ width: `${c.pct}%`, height: "100%", borderRadius: 3, background: c.color }} /></div>
                                  <span style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", fontFamily: "'JetBrains Mono',monospace" }}>{c.pct.toFixed(1)}%</span>
                                </div>
                              </td>
        ...
        {auth.authenticated && !auth.checking && activeTab === "traffic" && (
          <>
            ...
            {trafficLoading ? (
              <div style={{ textAlign: "center", padding: "60px 0", color: "rgba(255,255,255,0.3)" }}>
                <div style={{ fontSize: 24, marginBottom: 12, animation: "spin 1s linear infinite" }}>⟳</div>Chargement des données trafic...
                <style>{`@keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }`}</style>
              </div>
            ) : trafficData && (
              <>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 16, marginBottom: 24 }}>
                  ...
                  <div style={{ background: "linear-gradient(135deg, rgba(99,102,241,0.12), rgba(99,102,241,0.04))", border: "1px solid rgba(99,102,241,0.15)", borderRadius: 14, padding: "20px 22px" }}>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 600 }}>Organic Search</div>
                  ...
                  <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, padding: "20px 22px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                    <EvoBadge value={trafficData.summary.avgOrganicVsM1} label="Moy. vs M-1" />
                  </div>
                  <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, padding: "20px 22px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                    <EvoBadge value={trafficData.summary.avgOrganicVsN1} label="Moy. vs N-1" />
                  </div>
                </div>
                ...
                <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, overflow: "hidden", marginBottom: 24 }}>
                  <div style={{ padding: "16px 22px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}><h3 style={{ fontSize: 14, fontWeight: 600, margin: 0 }}>Détail par client</h3></div>
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                      <thead>
                        <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                          {["Client", "Trafic global", "Organic Search", "vs M-1", "vs N-1"].map((h) => (
                            <th key={h} style={{ padding: "10px 16px", textAlign: h === "Client" ? "left" : "right", fontSize: 11, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "0.05em", fontWeight: 600 }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {trafficData.clients.map((c, ci) => (
                          <>
                            <tr key={c.id} onClick={() => setExpandedClient(expandedClient === c.id ? null : c.id)}
                              style={{ borderBottom: "1px solid rgba(255,255,255,0.03)", background: expandedClient === c.id ? "rgba(99,102,241,0.05)" : "transparent", cursor: "pointer", transition: "background 0.2s" }}>
                            ...
                            {expandedClient === c.id && (
                              <tr key={`${c.id}-expanded`}>
                                <td colSpan={5} style={{ padding: "0 16px 16px", background: "rgba(99,102,241,0.03)" }}>
                                  <div style={{ padding: "12px 0 4px", fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.5)" }}>Organic Search — Évolution mensuelle</div>
                ...
                <div style={{ background: "linear-gradient(135deg, rgba(99,102,241,0.06), rgba(99,102,241,0.02))", border: "1px solid rgba(99,102,241,0.12)", borderRadius: 14, padding: "24px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12, marginBottom: 16 }}>
                    <h3 style={{ fontSize: 15, fontWeight: 700, margin: 0, color: "#a5b4fc" }}>Récapitulatif — Évolution moyenne globale</h3>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <input type="date" value={summaryStartDate} onChange={(e) => setSummaryStartDate(e.target.value)} style={{ padding: "5px 8px", borderRadius: 6, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.04)", color: "#e2e2f0", fontSize: 11, fontFamily: "'JetBrains Mono',monospace" }} />
                      <span style={{ color: "rgba(255,255,255,0.3)", fontSize: 11 }}>→</span>
                      <input type="date" value={summaryEndDate} onChange={(e) => setSummaryEndDate(e.target.value)} style={{ padding: "5px 8px", borderRadius: 6, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.04)", color: "#e2e2f0", fontSize: 11, fontFamily: "'JetBrains Mono',monospace" }} />
                      <button onClick={() => { if (summaryStartDate && summaryEndDate) fetchTraffic(summaryStartDate, summaryEndDate); }}
                        style={{ padding: "5px 14px", borderRadius: 6, border: "1px solid #6366f1", background: "rgba(99,102,241,0.15)", color: "#a5b4fc", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
                        Recalculer
                      </button>
                    </div>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
                    <div style={{ background: "rgba(0,0,0,0.2)", borderRadius: 12, padding: "20px", textAlign: "center" }}>
                      <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 600 }}>Moyenne Organic vs M-1</div>
                      <div style={{ fontSize: 32, fontWeight: 700, color: trafficData.summary.avgOrganicVsM1 >= 0 ? "#6ee7b7" : "#fca5a5", fontFamily: "'JetBrains Mono',monospace" }}>
                        {formatPct(trafficData.summary.avgOrganicVsM1)}
                      </div>
                      <div style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", marginTop: 4 }}>sur {trafficData.summary.clientCount} clients</div>
                    </div>
                    <div style={{ background: "rgba(0,0,0,0.2)", borderRadius: 12, padding: "20px", textAlign: "center" }}>
                      <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 600 }}>Moyenne Organic vs N-1</div>
                      <div style={{ fontSize: 32, fontWeight: 700, color: trafficData.summary.avgOrganicVsN1 >= 0 ? "#6ee7b7" : "#fca5a5", fontFamily: "'JetBrains Mono',monospace" }}>
                        {formatPct(trafficData.summary.avgOrganicVsN1)}
                      </div>
                      <div style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", marginTop: 4 }}>sur {trafficData.summary.clientCount} clients</div>
                    </div>
                    <div style={{ background: "rgba(0,0,0,0.2)", borderRadius: 12, padding: "20px", textAlign: "center" }}>
                      <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 600 }}>Total Organic Search</div>
                      <div style={{ fontSize: 32, fontWeight: 700, color: "#a5b4fc", fontFamily: "'JetBrains Mono',monospace" }}>
                        {formatNum(trafficData.summary.totalOrganic)}
                      </div>
                      <div style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", marginTop: 4 }}>sessions organiques</div>
                    </div>
                  </div>
                </div>
              </>
